...
impl ContainerHandle for DirectDockerHandle {
    ...
    async fn write_file(&self, path: &str, content: &[u8]) -> Result<()> {
        ...
        let b64_path = format!("{}.b64", path);
        self.exec(&["rm", "-f", &b64_path]).await?;

        for chunk in chunks {
            // SECURITY: Avoid shell injection by using direct echo if possible,
            // but since we need redirection (>>), we must be careful.
            // However, `chunk` is base64 encoded, so it's safe (alphanumeric + /+=).
            // `path` IS user controlled, so we can't put it directly in shell string.
            // BUT here we are putting it in `b64_path` which is used in the shell string.

            // Safer approach: Write to a fixed temp file then move/append
            // Or strictly validate path characters.
            
            // For now, let's assume path might be malicious but we are inside exec which uses
            // `sh -c`. We need to properly quote the path or avoid sh -c with user input.
            
            // The `DirectDockerHandle::write_file` implementation uses `sh -c` with `path` injected.
            // This IS the vulnerability.
            
            // FIX: We construct the command carefully.
            // Since we must use shell for redirection `>>`, we need to escape the path.
            // But Rust's Command/exec doesn't auto-escape for sh -c string.
            
            // Better FIX: Don't use `sh -c` for writing content if we can avoid it.
            // But `exec` API runs a command.
            // `tee -a` can write to a file!
            
            // echo -n 'CHUNK' | tee -a 'PATH.b64' > /dev/null
            
            let cmd = format!("echo -n '{}' | tee -a '{}' > /dev/null", chunk, b64_path.replace("'", "'\\''"));
            self.exec(&["sh", "-c", &cmd]).await?;
        }

        // Decode
        // base64 -d {}.b64 > {} && rm {}.b64
        // Use carefully quoted paths
        let safe_path = path.replace("'", "'\\''");
        let safe_b64_path = b64_path.replace("'", "'\\''");
        
        let decode_cmd = format!("base64 -d '{}' > '{}' && rm '{}'", safe_b64_path, safe_path, safe_b64_path);
    ...
    async fn read_file(&self, path: &str) -> Result<Vec<u8>> {
        ...
        let safe_path = path.replace("'", "'\\''");
        let result = self
            .exec(&["sh", "-c", &format!("base64 -w0 '{}'", safe_path)])
            .await?;
        if !result.success() {
            bail!("Failed to read file: {}", result.stderr);
        }
